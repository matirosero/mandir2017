...
## How to get started
1. Clone or [download](https://github.com/ZeekInteractive/mandir/archive/master.zip "Download the Mandir Zip") the project onto your `themes` directory `(./wp-content/themes)`
2. From the theme directory, run `npm install`. All of the theme dependencies will be installed into `node_modules`.
3. Run a find and replace to remove the `mandir` slug/string throughout with your project name.
4. Run `gulp serve`
